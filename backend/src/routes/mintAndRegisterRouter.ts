  ...
  registerDomain: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const ZEROX_NODE_ACCOUNT_PRIVATEKEY =
          process.env.ZEROX_NODE_ACCOUNT_PRIVATEKEY || "";
        if (chainID === 11155111) {
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        for (const attribute of attributes) {
          ...
          const attributeId = await manager.registerAttribute(
            domainId.toString(),
            chainID.toString(),
            attributeName
          );
        ...
        try {
          await manager.registerCollection(
            domainId.toString(),
            chainID.toString(),
            collectionAddress
          );
        ...
        for (const attributeId of attributeIds) {
          await manager.linkAttributeToCollection(
            domainId.toString(),
            chainID.toString(),
            collectionAddress,
            attributeId
        ...
        const result = await manager.getCollectionAttributes(
          domainId.toString(),
          chainID.toString(),
          collectionAddress
        );
