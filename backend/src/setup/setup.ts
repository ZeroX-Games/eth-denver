...
async function main() {
    ...
    /// Register Domain for Fighter Game - Base
    let domainId = await registerDomain(BASESEPchainID)
    if (domainId?.domainId) {
        console.log("Domain ID: ", domainId.domainId, "Base Sepolia - Fighter Game");
        await registerCollectionGame(domainId.domainId, BASESEPchainID)
    }

    /// Register Domain for Fighter Game - Arb
    domainId = await registerDomain(ARBSEPchainID)
    if (domainId?.domainId) {
        console.log("Domain ID: ", domainId.domainId, "ARB Sepolia - Fighter Game");
        await registerCollectionGame(domainId.domainId, ARBSEPchainID)
    }

    /// Register Domain for Fighter Game - Linea Goerli
    domainId = await registerDomain(LINEAGOEchainID)
    if (domainId?.domainId) {
        console.log("Domain ID: ", domainId.domainId, "Linea Goerli - Fighter Game");
        await registerCollectionGame(domainId.domainId, LINEAGOEchainID)
    }

    /// Register Domain for PokeCard -  Arbitrum Sepolia
    domainId = await registerDomain(ARBSEPchainID)
    if (domainId?.domainId) {
        console.log("Domain ID: ", domainId.domainId, "Arbitrum Sepolia - PokeCard");
        await registerCollectionPokeCard(domainId.domainId, ARBSEPchainID)
    }

    /// Register Domain for Social - Linea GOERLI
    domainId = await registerDomain(LINEAGOEchainID)
    if (domainId?.domainId) {
        console.log("Domain ID: ", domainId.domainId, "Linea Goerli - Social");
        await registerCollectionSocial(domainId.domainId, LINEAGOEchainID)
    }
